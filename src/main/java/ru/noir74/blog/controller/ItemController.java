...
public class ItemController {
    ...
    public String getPage(Model model,
        ...
        List<ItemDtoRespBrief> items = List.of();
        model.addAttribute("items", items);
        return "items";
    }

    @GetMapping("/{id}")
    public ItemDtoRespDetail get(@PathVariable Integer id) {
        log.info("GET /{}", id);
        return itemMapper.model2dtoResp(itemService.get(id));
    }

    @PostMapping
    public ItemDtoRespDetail create(@Validated(OnCreate.class) @RequestBody ItemDtoReq dtoReq) {
